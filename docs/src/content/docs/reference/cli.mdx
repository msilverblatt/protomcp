...
**Repeatable.** Configure authentication for network transports (SSE, HTTP, WebSocket). Skipped for stdio.
...
pmcp run tools.py --transport sse --auth token:MY_TOKEN

# Require API key
pmcp run tools.py --transport sse --auth apikey:SERVICE_KEY

# Multiple (all must pass)
pmcp run tools.py --transport sse --auth token:MY_TOKEN --auth apikey:SERVICE_KEY
```

The environment variable must be set at startup. See [Authentication](/guides/auth/) for details.
...
```sh
# Python
pmcp dev tools.py --transport sse

# TypeScript
pmcp dev tools.ts --transport sse
```
...
```sh
# Python
pmcp run tools.py --transport sse --host 0.0.0.0

# TypeScript
pmcp run tools.ts --transport sse --host 0.0.0.0
```
...
```sh
# Python
pmcp run tools.py --transport sse --port 9000

# TypeScript
pmcp run tools.ts --transport sse --port 9000
```
