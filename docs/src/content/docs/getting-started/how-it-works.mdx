...
```mermaid
graph LR
    A[MCP Host<br/>Claude Desktop] <-->|MCP protocol<br/>stdio / HTTP / SSE| B[protomcp<br/>Go binary]
...
The `Envelope` message wraps all request and response types in a `oneof` field. See the [Protobuf Spec](/reference/protobuf-spec/) for full details.

---

## Transports

protomcp supports five transports for the MCP host side:

| Transport | Flag | Use case |
|-----------|------|----------|
| stdio | `--transport stdio` | MCP clients that spawn subprocesses (default) |
| HTTP SSE | `--transport sse` | Web-based or remote MCP clients |
| Streamable HTTP | `--transport http` | Modern HTTP clients |
| WebSocket | `--transport ws` | Real-time bidirectional clients |
| In-process | `--transport inproc` | Testing and embedding |
