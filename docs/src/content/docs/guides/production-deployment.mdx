...
[Service]
Type=simple
ExecStart=/usr/local/bin/pmcp run /opt/mytools/tools.py --transport sse --host 127.0.0.1 --port 8080
# For TypeScript: ExecStart=/usr/local/bin/pmcp run /opt/mytools/tools.ts --transport sse --host 127.0.0.1 --port 8080
...
EXPOSE 8080
CMD ["pmcp", "run", "tools.py", "--transport", "sse", "--host", "0.0.0.0", "--port", "8080"]
```
...
```sh
# Python
pmcp run auth_tools.py --transport sse --port 8081 &
pmcp run data_tools.py --transport sse --port 8082 &

# TypeScript
pmcp run auth_tools.ts --transport sse --port 8081 &
pmcp run data_tools.ts --transport sse --port 8082 &
```
